...
const PerformanceCustomizer = () => {
  const [selectedWheel, setSelectedWheel] = useState('rad-45');
  ...
  const wheelOptions: ConfigOption[] = [
    { id: 'rad-45', title: 'RAD45', description: '28mm wide, 50mm deep carbon aero wheels (Standard)' },
    { id: 'dtswiss-x1900', title: 'DTSwiss X1900 28', description: 'Superlight Aluminium wheels for climbing' }
  ];
  ...
  const drivetrainOptions: ConfigOption[] = [
    ...
    { id: 'shimano-grx', title: 'Shimano GRX 1x xD', description: '10-42T 12-speed for versatile riding' }
  ];
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const result = await sendEmail({
        subject: `Performance Road Bike Customization Request: ${formState.name}`,
        message: `
Name: ${formState.name}
...
Build Options:
Wheelset: ${wheelOptions.find(opt => opt.id === selectedWheel)?.title || selectedWheel}
Drivetrain: ${drivetrainOptions.find(opt => opt.id === selectedDrivetrain)?.title || selectedDrivetrain}
Handlebar: ${handlebarOptions.find(opt => opt.id === selectedHandlebar)?.title || selectedHandlebar}
...
<div style="font-family: Arial, sans-serif; padding: 20px; color: #333;">
  <h2 style="color: #f87f01;">New Performance Road Bike Customization Request</h2>
  ...
  <table style="width: 100%; border-collapse: collapse; margin-top: 10px;">
    <tr style="background-color: #f5f5f5;">
      ...
      <th style="padding: 8px; text-align: left; border: 1px solid #ddd;">Selection</th>
      <th style="padding: 8px; text-align: left; border: 1px solid #ddd;">Description</th>
    </tr>
    <tr>
      <td style="padding: 8px; border: 1px solid #ddd;">Wheelset</td>
      <td style="padding: 8px; border: 1px solid #ddd;">${wheelOptions.find(opt => opt.id === selectedWheel)?.title || selectedWheel}</td>
      <td style="padding: 8px; border: 1px solid #ddd;">${wheelOptions.find(opt => opt.id === selectedWheel)?.description || ''}</td>
    </tr>
    <tr>
      <td style="padding: 8px; border: 1px solid #ddd;">Drivetrain</td>
      <td style="padding: 8px; border: 1px solid #ddd;">${drivetrainOptions.find(opt => opt.id === selectedDrivetrain)?.title || selectedDrivetrain}</td>
      <td style="padding: 8px; border: 1px solid #ddd;">${drivetrainOptions.find(opt => opt.id === selectedDrivetrain)?.description || ''}</td>
    </tr>
    <tr>
      <td style="padding: 8px; border: 1px solid #ddd;">Handlebar</td>
      <td style="padding: 8px; border: 1px solid #ddd;">${handlebarOptions.find(opt => opt.id === selectedHandlebar)?.title || selectedHandlebar}</td>
      <td style="padding: 8px; border: 1px solid #ddd;">${handlebarOptions.find(opt => opt.id === selectedHandlebar)?.description || ''}</td>
    </tr>
  </table>
  ...
  <p>${formState.comments}</p>
      ...
      if (result.success) {
        setFormStatus({ 
          submitted: true, 
          error: false,
          message: 'Your performance bike customization request has been submitted successfully. Our team will contact you shortly.'
        });
        // Reset form
        setFormState({
          name: '',
        ...
        setSelectedWheel('rad-45');
        setSelectedDrivetrain('sram-red');
        setSelectedHandlebar('standard');
      } else {
        throw new Error(result.error || 'Failed to submit request');
      }
    } catch (error) {
      console.error('Error submitting performance bike request:', error);
      setFormStatus({ 
        submitted: true, 
        error: true,
            ...
            <form className="space-y-6 sm:space-y-8" onSubmit={handleSubmit}>
              ...
              <div className="pt-4 text-center">
                <button type="submit" className="inline-block w-full sm:w-auto" disabled={isSubmitting}>
                  <div className="rhombus-btn-primary relative overflow-hidden group w-full">
